...
const RestaurantRequests = () => {
    ...
    try {
      ...
      if (fetchAll) {
        ...
        const [pendingRes, approvedRes, rejectedRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL}/api/restaurants/admin/pending-registrations`, {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get(`${import.meta.env.VITE_API_URL}/api/restaurants/admin/approved-registrations`, {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get(`${import.meta.env.VITE_API_URL}/api/restaurants/admin/rejected-registrations`, {
            headers: { Authorization: `Bearer ${token}` }
          })
        ]);
        ...
        switch (activeTab) {
          case 'pending':
            endpoint = `${import.meta.env.VITE_API_URL}/api/restaurants/admin/pending-registrations`;
            break;
          case 'approved':
            endpoint = `${import.meta.env.VITE_API_URL}/api/restaurants/admin/approved-registrations`;
            break;
          case 'rejected':
            endpoint = `${import.meta.env.VITE_API_URL}/api/restaurants/admin/rejected-registrations`;
            break;
          default:
            endpoint = `${import.meta.env.VITE_API_URL}/api/restaurants/admin/pending-registrations`;
        }
  ...
  const handleStatusUpdate = async (restaurantId, status) => {
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/restaurants/admin/registration-status`,
        { 
          restaurantId,
          status 
        },
