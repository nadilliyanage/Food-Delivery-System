...
const CurrentDeliveries = () => {
    ...
    if (!isTrackingLocation) {
      try {
        ...
        if (retryCount < MAX_RETRIES) {
          ...
          setLocationError(`Attempting to get location (${retryCount + 1}/${MAX_RETRIES})...`);
        } else {
          ...
          switch (error.code) {
            ...
            case error.POSITION_UNAVAILABLE:
              ...
              break;
            case error.TIMEOUT:
              errorMessage += 'Location request timed out after multiple attempts. Please check your GPS and try again.';
              break;
            default:
              errorMessage += error.message;
          }
  ...
  const deliveryLocation = activeDelivery?.order?.deliveryAddress?.latitude && activeDelivery?.order?.deliveryAddress?.longitude
    ? {
  ...
  const mapCenter = deliveryLocation || userLocation || { lat: 6.927079, lng: 79.861244 }; // Default to Colombo
          ...
          {deliveries.map((delivery) => (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {activeDelivery?._id === delivery._id && delivery.status === "On the Way" && (
                  <div className="mt-4">
                    <div className="w-full h-[300px] rounded-lg overflow-hidden">
                      ...
                      >
                        ...
                        {userLocation && (
                          <Marker position={[userLocation.lat, userLocation.lng]} icon={driverIcon}>
                            <Popup>
                              <div>
                        ...
                        {deliveryLocation && (
                          <Marker position={[deliveryLocation.lat, deliveryLocation.lng]} icon={deliveryIcon}>
                            <Popup>
                              <div>
                                <p className="font-semibold">{activeDelivery.order.customer.name}</p>
                                <p>{activeDelivery.order.deliveryAddress.street}</p>
                                <p>{activeDelivery.order.deliveryAddress.city}</p>
                              </div>
                            </Popup>
                          </Marker>
                        )}
